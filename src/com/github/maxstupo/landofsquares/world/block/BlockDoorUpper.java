 ...
 *
 */
public class BlockDoorUpper extends BlockDoorBase {

    public BlockDoorUpper(int id) {
        super(id, BlockMaterial.wood);
        setSpriteKey("block_door_upper");
    }

    @Override
    public void onDestroyedBy(World world, int x, int y, int data, AbstractLivingEntity e) {
        super.onDestroyedBy(world, x, y, data, e);
        if (world.getBlock(x, y + 1).compareID(Block.door_lower))
    ...
    public void onBlockRightclick(AbstractLivingEntity e, World world, Vector2i handPos, int data) {
        ...
        if (data == 0 || data == 1) {
            ...
            if (world.getBlock(handPos.x, handPos.y + 1).compareID(Block.door_lower)) {
    ...
    public Sprite getSprite(int data) {
        return AssetManager.get().getSprite((data == 1) ? "block_door_lower" : spriteKey);
    }
